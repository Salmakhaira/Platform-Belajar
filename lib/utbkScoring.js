...
// Benar: skor berdasarkan difficulty (easy/medium/hard)
...
export function calculateIRTScore(questions, answers) {
  ...
  // Weight berdasarkan difficulty (IRT simplified)
  ...
  questions.forEach(q => {
    ...
    if (!userAnswer) {
      ...
      // Benar = dapat poin sesuai difficulty
      totalScore += weight
      correct++
    } else {
      // Salah = 0 poin (TIDAK MINUS!)
      wrong++
    }
  })
...
export function calculatePerSubmateri(questions, answers) {
  ...
  // Calculate normalized scores for each submateri
...
function generateRecommendations(avgScore, trend, submateriAnalysis) {
  ...
  if (avgScore >= 700) {
    ...
    recommendations.push({
      type: 'overall',
      message: 'Performa cukup. Fokus pada penguasaan materi dasar dan latihan rutin.'
    })
  } else {
    recommendations.push({
      type: 'overall',
      message: 'Perlu peningkatan signifikan. Pelajari konsep dasar dan perbanyak latihan.'
    })
  }
